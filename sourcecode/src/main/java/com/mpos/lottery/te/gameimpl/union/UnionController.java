...
public class UnionController {
    ...
    @RequestMap("{transType:202,gameType:8}")
    public void enquiryTicket(Context request, Context response) throws ApplicationException {
        UnionTicket ticket = (UnionTicket) request.getModel();
        ticket = (UnionTicket) this.getTicketService().enquiry(response, ticket, true);

        response.setModel(ticket);
    }

    @RequestMap("{transType:301,gameType:8}")
    ...
    @RequestMap("{transType:302,gameType:8}")
    ...
    @RequestMap("{transType:205,gameType:8}")
